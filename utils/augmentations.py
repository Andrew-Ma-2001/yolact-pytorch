...
class RandomSampleCrop(object):
    ...
    def __call__(self, image, masks, boxes=None, labels=None):
        ...
        while True:
            ...
            # flattened_options = [item for sublist in self.sample_options for item in sublist if item is not None]
            # mode = random.choice(flattened_options)
            mode = random.choice(len(self.sample_options))
            mode = self.sample_options[mode]
            # mode = random.choice([option for option in self.sample_options if option is not None])
            if mode is None:
                return image, masks, boxes, labels
